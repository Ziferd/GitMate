...
# ============================================================
# GitMate v4.0 — GitHub CLI Assistant for Termux
# Developer By: Ziferd
# Bilingual (EN / ID) — set GITMATE_LANG=id for Indonesian
# ============================================================

set -euo pipefail

# Language Setup
GITMATE_LANG="${GITMATE_LANG:-en}"

# Translation tables (sama seperti sebelumnya, tidak diubah)
if [ "$GITMATE_LANG" = "id" ]; then
    T_DEPENDENCIES_OK="Semua dependensi terpenuhi!"
    T_DEPENDENCIES_MISSING="Dependensi berikut belum terinstall:"
    T_RUN_SETUP="Jalankan: gitmate setup"
    ...
    T_REPO_INIT="Repo dibuat. Tinggal tambah remote: gitmate remote add origin <url>"
    ...
    T_DAEMON_START="GitMate daemon jalan (sync tiap"
    ...
    T_SETUP_DONE="Setup selesai. Coba: gitmate help"
    T_LOGIN_GH="Udah login GitHub."
    T_LOGIN_GH_PROMPT="Login ke GitHub:"
    T_HELP_LINE="Ketik 'gitmate help' buat liat daftar perintah."
    ...
    T_REPO_INIT="Repo initialized. Add remote: gitmate remote add origin <url>"
    T_BRANCH_CREATED="Branch"
    ...
    T_DAEMON_START="GitMate daemon running (sync every"
    ...
    T_SETUP_DONE="Setup complete. Try: gitmate help"
    T_LOGIN_GH="Already logged into GitHub."
    T_LOGIN_GH_PROMPT="Login to GitHub:"
    T_HELP_LINE="Type 'gitmate help' for command list."
...
banner() {
    echo ""
    echo "  =========================================="
    echo "      GitMate v4.0 - GitHub CLI Assistant for Termux"
...
# Core Commands (tidak diubah, sama seperti sebelumnya)
cmd_deploy() {
...
cmd_remote() {
    ...
    case "${1:-show}" in
        show)   git remote -v ;;
        add)    [ -z "$3" ] && { fail "Usage: gitmate remote add <name> <url>"; return 1; }
                git remote add "$2" "$3" && ok "$T_REMOTE_ADDED $2" ;;
        set)    [ -z "$3" ] && { fail "Usage: gitmate remote set <name> <url>"; return 1; }
                git remote set-url "$2" "$3" && ok "$T_REMOTE_UPDATED" ;;
        delete) [ -z "$2" ] && { fail "Usage: gitmate remote delete <name>"; return 1; }
...
cmd_branch() {
    ...
    case "${1:-list}" in
        list)   git branch -a ;;
        create) [ -z "$2" ] && { fail "Usage: gitmate branch create <name>"; return 1; }
                git checkout -b "$2" && ok "$T_BRANCH_CREATED '$2' $T_BRANCH_CREATED_END" ;;
        switch) [ -z "$2" ] && { fail "Usage: gitmate branch switch <name>"; return 1; }
                git checkout "$2" && ok "$T_BRANCH_SWITCHED '$2'" ;;
        delete) [ -z "$2" ] && { fail "Usage: gitmate branch delete <name>"; return 1; }
                git branch -d "$2" 2>/dev/null && ok "$T_BRANCH_DELETED '$2' $T_BRANCH_DELETED_END" ;;
        rename) [ -z "$2" ] || [ -z "$3" ] && { fail "Usage: gitmate branch rename <old> <new>"; return 1; }
...
cmd_merge() {
    check_git || return 1
    [ -z "$1" ] && { fail "Usage: gitmate merge <branch>"; return 1; }
...
cmd_reset() {
    ...
    case "$mode" in
        ...
        *)    fail "Usage: gitmate reset [soft|hard] [commit]" ;;
    esac
}
...
cmd_cherrypick() {
    check_git || return 1
    [ -z "$1" ] && { fail "Usage: gitmate cherry-pick <commit>"; return 1; }
...
cmd_bisect() {
    ...
    case "${1:-start}" in
        ...
        *)     fail "Usage: gitmate bisect start|good|bad|reset" ;;
    esac
}

# GitHub API (tidak diubah)
cmd_repo() {
    [ -z "$1" ] && { fail "Usage: gitmate repo <name> [public|private]"; return 1; }
    gh repo create "$1" --"${2:-public}" --clone && ok "$T_REPO_READY"
}

cmd_clone() {
    [ -z "$1" ] && { fail "Usage: gitmate clone <user/repo>"; return 1; }
...
cmd_gist() {
    case "${1:-list}" in
        list)   gh gist list ;;
        create) [ -z "$2" ] && { fail "Usage: gitmate gist create <file>"; return 1; }
                gh gist create "$2" -d "Gist from GitMate" && ok "$T_GIST_CREATED" ;;
        edit)   [ -z "$2" ] && { fail "Usage: gitmate gist edit <id>"; return 1; }
                gh gist edit "$2" && ok "$T_GIST_EDITED" ;;
        delete) [ -z "$2" ] && { fail "Usage: gitmate gist delete <id>"; return 1; }
...
cmd_issue() {
    case "${1:-list}" in
        list)   gh issue list ;;
        create) [ -z "$2" ] && { fail "Usage: gitmate issue create '<title>'"; return 1; }
                gh issue create -t "$2" && ok "$T_ISSUE_CREATED" ;;
        close)  [ -z "$2" ] && { fail "Usage: gitmate issue close <number>"; return 1; }
                gh issue close "$2" && ok "$T_ISSUE_CLOSED" ;;
        view)   [ -z "$2" ] && { fail "Usage: gitmate issue view <number>"; return 1; }
                gh issue view "$2" ;;
...
cmd_pr() {
    case "${1:-list}" in
        list)     gh pr list ;;
        create)   [ -z "$2" ] && { fail "Usage: gitmate pr create '<title>'"; return 1; }
                  gh pr create -t "$2" && ok "$T_PR_CREATED" ;;
        merge)    [ -z "$2" ] && { fail "Usage: gitmate pr merge <number>"; return 1; }
                  gh pr merge "$2" && ok "$T_PR_MERGED" ;;
        checkout) [ -z "$2" ] && { fail "Usage: gitmate pr checkout <number>"; return 1; }
                  gh pr checkout "$2" && ok "$T_PR_CHECKOUT" ;;
        review)   [ -z "$2" ] && { fail "Usage: gitmate pr review <number>"; return 1; }
...
cmd_release() {
    case "${1:-list}" in
        ...
        create) [ -z "$2" ] && { fail "Usage: gitmate release create <tag>"; return 1; }
                gh release create "$2" && ok "$T_RELEASE_CREATED" ;;
        delete) [ -z "$2" ] && { fail "Usage: gitmate release delete <tag>"; return 1; }
...
cmd_actions() {
    case "${1:-list}" in
        list)    gh run list ;;
        watch)   [ -z "$2" ] && { fail "Usage: gitmate actions watch <run_id>"; return 1; }
                 gh run watch "$2" && ok "$T_ACTIONS_WATCH" ;;
        trigger) [ -z "$2" ] && { fail "Usage: gitmate actions trigger <workflow>"; return 1; }
...
cmd_backup() {
    local dest="${1:-$HOME/gitmate-backups}"
    mkdir -p "$dest"
...
cmd_webhook() {
    ...
    curl -s -X POST "$WEBHOOK_URL" -H "Content-Type: application/json" \
      -d '{"event":"ping","tool":"gitmate","timestamp":"'"$(date -Iseconds)"'"}' && ok "$T_WEBHOOK_TEST"
}

cmd_keygen() {
    local email="${1:-gitmate@termux.local}"
    local key="$HOME/.ssh/id_ed25519_gitmate"
    mkdir -p "$HOME/.ssh"
...
cmd_undo() {
    case "${1:-commit}" in
        ...
        file)   [ -z "$2" ] && { fail "Usage: gitmate undo file <name>"; return 1; }
...
cmd_help() {
    echo ""
    echo "  =========================================="
    echo "      GitMate v4.0 - GitHub CLI Assistant"
    echo "      Developer By: Ziferd"
    echo "  =========================================="
    echo ""

    [ "$GITMATE_LANG" = "id" ] && echo "  Perintah Dasar:" || echo "  Basic Commands:"
    [ "$GITMATE_LANG" = "id" ] && echo "  gitmate deploy  \"pesan\" [branch]     Stage, commit, push" || echo "  gitmate deploy  \"msg\" [branch]       Stage, commit, push"
    echo ""
    ...
    [ "$GITMATE_LANG" = "id" ] && echo "  gitmate status                       Status repo" || echo "  gitmate status                       Repo status"
    [ "$GITMATE_LANG" = "id" ] && echo "  gitmate log                          Riwayat commit" || echo "  gitmate log                          Commit history"
    [ "$GITMATE_LANG" = "id" ] && echo "  gitmate diff                         Perubahan" || echo "  gitmate diff                         Changes"
    [ "$GITMATE_LANG" = "id" ] && echo "  gitmate stash list|save|pop|drop    Simpan perubahan sementara" || echo "  gitmate stash list|save|pop|drop    Temporary stash"
    [ "$GITMATE_LANG" = "id" ] && echo "  gitmate tag list|create|push|delete  Kelola tag" || echo "  gitmate tag list|create|push|delete  Manage tags"
    [ "$GITMATE_LANG" = "id" ] && echo "  gitmate remote show|add|set|delete   Kelola remote URL" || echo "  gitmate remote show|add|set|delete   Manage remote URLs"
    [ "$GITMATE_LANG" = "id" ] && echo "  gitmate init    [nama]               Buat repo baru" || echo "  gitmate init    [name]               Create new repo"
    echo ""

    [ "$GITMATE_LANG" = "id" ] && echo "  Branch & Merge:" || echo "  Branch & Merge:"
    [ "$GITMATE_LANG" = "id" ] && echo "  gitmate branch list|create|switch|delete|rename  Kelola branch" || echo "  gitmate branch list|create|switch|delete|rename  Manage branches"
    [ "$GITMATE_LANG" = "id" ] && echo "  gitmate merge   <branch>             Gabungkan branch" || echo "  gitmate merge   <branch>             Merge branch"
    [ "$GITMATE_LANG" = "id" ] && echo "  gitmate fetch                        Ambil metadata remote" || echo "  gitmate fetch                        Fetch remote metadata"
    echo ""

    [ "$GITMATE_LANG" = "id" ] && echo "  Rewrite History:" || echo "  Rewrite History:"
    [ "$GITMATE_LANG" = "id" ] && echo "  gitmate reset soft|hard [target]     Reset ke commit" || echo "  gitmate reset soft|hard [target]     Reset to commit"
    [ "$GITMATE_LANG" = "id" ] && echo "  gitmate reflog                       Penyelamat HEAD" || echo "  gitmate reflog                       HEAD recovery"
    [ "$GITMATE_LANG" = "id" ] && echo "  gitmate squash [jumlah]              Gabung commit terbaru" || echo "  gitmate squash [count]              Squash recent commits"
    [ "$GITMATE_LANG" = "id" ] && echo "  gitmate rebase [branch]              Rebase ke branch lain" || echo "  gitmate rebase [branch]             Rebase onto branch"
    [ "$GITMATE_LANG" = "id" ] && echo "  gitmate cherry-pick <commit>         Ambil commit spesifik" || echo "  gitmate cherry-pick <commit>         Pick specific commit"
    [ "$GITMATE_LANG" = "id" ] && echo "  gitmate bisect start|good|bad|reset  Cari penyebab bug" || echo "  gitmate bisect start|good|bad|reset  Find bug cause"
    echo ""

    [ "$GITMATE_LANG" = "id" ] && echo "  GitHub API:" || echo "  GitHub API:"
    [ "$GITMATE_LANG" = "id" ] && echo "  gitmate repo  <name> [public|private] Buat repo GitHub" || echo "  gitmate repo  <name> [public|private] Create GitHub repo"
    [ "$GITMATE_LANG" = "id" ] && echo "  gitmate clone <user/repo>            Clone repo GitHub" || echo "  gitmate clone <user/repo>            Clone GitHub repo"
    [ "$GITMATE_LANG" = "id" ] && echo "  gitmate gist list|create|edit|delete Kelola Gist" || echo "  gitmate gist list|create|edit|delete Manage Gists"
    [ "$GITMATE_LANG" = "id" ] && echo "  gitmate issue list|create|close|view Kelola Issue" || echo "  gitmate issue list|create|close|view Manage Issues"
    [ "$GITMATE_LANG" = "id" ] && echo "  gitmate pr list|create|merge|checkout|review Kelola PR" || echo "  gitmate pr list|create|merge|checkout|review Manage PRs"
    [ "$GITMATE_LANG" = "id" ] && echo "  gitmate release list|create|delete   Kelola Release" || echo "  gitmate release list|create|delete   Manage Releases"
    [ "$GITMATE_LANG" = "id" ] && echo "  gitmate actions list|watch|trigger   GitHub Actions" || echo "  gitmate actions list|watch|trigger   GitHub Actions"
    echo ""

    [ "$GITMATE_LANG" = "id" ] && echo "  Automation:" || echo "  Automation:"
    [ "$GITMATE_LANG" = "id" ] && echo "  gitmate backup [folder]              Backup semua repo" || echo "  gitmate backup [folder]              Backup all repos"
    [ "$GITMATE_LANG" = "id" ] && echo "  gitmate stats                        Statistik kontribusi" || echo "  gitmate stats                        Contribution stats"
    [ "$GITMATE_LANG" = "id" ] && echo "  gitmate daemon [detik]               Auto-sync tiap N detik" || echo "  gitmate daemon [seconds]             Auto-sync every N sec"
    [ "$GITMATE_LANG" = "id" ] && echo "  gitmate webhook                      Uji coba webhook" || echo "  gitmate webhook                      Test webhook"
    echo ""

    [ "$GITMATE_LANG" = "id" ] && echo "  Setup & Config:" || echo "  Setup & Config:"
    [ "$GITMATE_LANG" = "id" ] && echo "  gitmate setup                        Install dependensi" || echo "  gitmate setup                        Install dependencies"
    [ "$GITMATE_LANG" = "id" ] && echo "  gitmate config show|set|remote       Atur konfigurasi git" || echo "  gitmate config show|set|remote       Configure git settings"
    [ "$GITMATE_LANG" = "id" ] && echo "  gitmate undo commit|file|hard        Batalkan perubahan" || echo "  gitmate undo commit|file|hard        Undo changes"
    [ "$GITMATE_LANG" = "id" ] && echo "  gitmate keygen [email]               Generate SSH key" || echo "  gitmate keygen [email]               Generate SSH key"
    [ "$GITMATE_LANG" = "id" ] && echo "  gitmate help                         Lihat bantuan ini" || echo "  gitmate help                         Show this help"
    echo ""

    echo "  =========================================="
    [ "$GITMATE_LANG" = "id" ] && echo "  Tips: export GITMATE_LANG=id untuk Bahasa Indonesia" || echo "  Tips: export GITMATE_LANG=id for Indonesian"
    echo "  =========================================="
    echo ""
}
