...
# Translation tables (tidak diubah)
...
banner() {
    echo ""
    echo -e "\033[1;36m  GitMate v4.0\033[0m - GitHub CLI Assistant for Termux"
...
cmd_help() {
    echo ""
    echo "╭───────────❍「 GITMATE v4.0 」❍"
    [ "$GITMATE_LANG" = "id" ] && echo "│ CLI Assistant untuk GitHub" || echo "│ GitHub CLI Assistant for Termux"
    echo "│ Developer By: Ziferd"
    echo "╰─────────────❍"
    echo ""
    ...
    [ "$GITMATE_LANG" = "id" ] && echo "│ • gitmate stash list|save|pop|drop  → Simpan sementara" || echo "│ • gitmate stash list|save|pop|drop  → Temporary stash"
    [ "$GITMATE_LANG" = "id" ] && echo "│ • gitmate tag list|create|push|delete → Kelola tag" || echo "│ • gitmate tag list|create|push|delete → Manage tags"
    [ "$GITMATE_LANG" = "id" ] && echo "│ • gitmate remote show|add|set|delete → Kelola remote" || echo "│ • gitmate remote show|add|set|delete → Manage remotes"
    ...
    [ "$GITMATE_LANG" = "id" ] && echo "│ • gitmate fetch                      → Ambil metadata" || echo "│ • gitmate fetch                      → Fetch metadata"
    echo "╰─────────────❍"
    echo ""
    ...
    [ "$GITMATE_LANG" = "id" ] && echo "│ • gitmate squash [jumlah]            → Gabung commit" || echo "│ • gitmate squash [count]            → Squash commits"
    [ "$GITMATE_LANG" = "id" ] && echo "│ • gitmate rebase [branch]            → Rebase" || echo "│ • gitmate rebase [branch]           → Rebase onto"
    [ "$GITMATE_LANG" = "id" ] && echo "│ • gitmate cherry-pick <commit>       → Ambil commit" || echo "│ • gitmate cherry-pick <commit>       → Pick commit"
    [ "$GITMATE_LANG" = "id" ] && echo "│ • gitmate bisect start|good|bad|reset → Cari bug" || echo "│ • gitmate bisect start|good|bad|reset → Find bug"
    echo "╰─────────────❍"
    echo ""
    ...
    [ "$GITMATE_LANG" = "id" ] && echo "│ • gitmate clone <user/repo>          → Clone repo" || echo "│ • gitmate clone <user/repo>          → Clone repo"
    ...
    [ "$GITMATE_LANG" = "id" ] && echo "│ • gitmate daemon [detik]             → Auto-sync" || echo "│ • gitmate daemon [seconds]           → Auto-sync"
    [ "$GITMATE_LANG" = "id" ] && echo "│ • gitmate webhook                    → Uji webhook" || echo "│ • gitmate webhook                    → Test webhook"
    echo "╰─────────────❍"
    echo ""

    echo "╭───────────❍「 SETUP & CONFIG 」❍"
    [ "$GITMATE_LANG" = "id" ] && echo "│ • gitmate setup                      → Install dependensi" || echo "│ • gitmate setup                      → Install dependencies"
    [ "$GITMATE_LANG" = "id" ] && echo "│ • gitmate config show|set|remote     → Konfigurasi git" || echo "│ • gitmate config show|set|remote     → Configure git"
    ...
    [ "$GITMATE_LANG" = "id" ] && echo "│ • gitmate help                       → Bantuan ini" || echo "│ • gitmate help                       → This help"
    echo "╰─────────────❍"
    echo ""

    echo "╭───────────❍「 TIPS 」❍"
    [ "$GITMATE_LANG" = "id" ] && echo "│ • export GITMATE_LANG=id untuk Bahasa Indonesia" || echo "│ • export GITMATE_LANG=id for Indonesian"
